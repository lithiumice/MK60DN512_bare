...
)
{
    ...
    if (res == FR_OK)
    {
        ...
        if (mode & (FA_CREATE_ALWAYS | FA_OPEN_ALWAYS | FA_CREATE_NEW))
        {
            ...
            {
                dw = 6000;//get_fattime();					/* Created time */
    ...
    if (res == FR_OK)
    {
        if (fp->flag & FA__WRITTEN)  	/* Has the file been written? */
        {
            ...
            if (res == FR_OK)
            {
                ...
                tm = 6000;//get_fattime();							/* Update updated time */
...
)
{
    ...
    DIR dj;
    BYTE *dir, n;
    DWORD dsc, dcl, pcl, tm = 6000;//get_fattime();
    DEF_NAMEBUF;
    res = chk_mounted(&path, &dj.fs, 1);
    if (res == FR_OK)
    {
...
)
{
    ...
    n = 6000;//get_fattime();						/* Use current time as VSN */
    if (fmt == FS_FAT32)
    {
