...
void cmt_pwm_init(uint16 freq, uint32 duty)
{
    uint32 temp_clk;
    ...
    //计算高低电平的计数次数
    temp_low_num = ((uint64)cmt_period * (CMT_PRECISON - duty) / CMT_PRECISON);
    temp_high_num = ((uint64)cmt_period * (duty) / CMT_PRECISON);
    //设置低电平时间
    temp_low_num--;
...
//  Sample usage:               cmt_pwm_init(50,50);    // 频率50HZ，占空比为百分之（50/CMT_PRECISON *100）;
//-------------------------------------------------------------------------------------------------------------------
void cmt_pwm_freq(uint16 freq, uint32 duty)
{
    uint32 temp_clk;
    uint32 temp_high_num, temp_low_num;
    uint32 temp_div;
